...
from [Conventional Commits](https://www.conventionalcommits.org/), so its sections are commit
types (Features, Bug Fixes, Performance Improvements, Documentation) rather than Keep a
Changelog's categories. Versioning follows
...
* **investigate:** `investigation.max_tokens_per_investigation` is now a CUMULATIVE ceiling over one investigation's model tokens (provider-reported input + output, loop AND verify), not a check on the next request's estimated size. **Its default is raised from 100000 to 400000** to suit the new meaning — read as a whole-run budget the old value funds only four or five steps. A quarter of it (100000 at the default) additionally bounds any single request, and mid-loop compaction triggers at 70% of that quarter (70000); both are the values in force before, so what one request may cost changes for nobody. If you set the key explicitly your value is left untouched and is now read as a run budget — set it to the total you will pay per incident, or `-1` to opt out (`0` applies the default, it does not disable). The check compares the projected total and still sends the request that crosses, so budget ~1.25x the number you set. Runs that hit it report result="budget_exceeded" with reason="tokens_total" and deliver an unresolved stub naming the ceiling. Full migration: https://runlore.io/docs/operations/upgrade-uninstall/
