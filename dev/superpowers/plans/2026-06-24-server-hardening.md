...
- [x] Test: `newHTTPServer(":0", h)` returns a `*http.Server` with all five fields
  (`ReadHeaderTimeout`, `ReadTimeout`, `WriteTimeout`, `IdleTimeout`,
  `MaxHeaderBytes`) non-zero.
- [x] Impl: extract `newHTTPServer(addr string, h http.Handler) *http.Server` with
...
- [x] Test: POST `/webhook/alertmanager` with a >1 MiB body → 413; reaffirm
  malformed → 400 and valid → 202 (existing tests stay green).
- [x] Impl: `r.Body = http.MaxBytesReader(w, r.Body, 1<<20)` before
...
- [x] Test: table over {model configured y/n} × {token "" / set} asserting the
...
- [x] Impl: after reading `webhookToken`, if `modelConfigured(cfg) &&
...
- [x] Test: `helm template deploy/helm/runlore` (default) → ingress has **no**
...
- [x] Impl: in the ingress rule, `{{- with .Values.networkPolicy.ingressFrom }}from:
...
### Task 5: Final gate
- [x] `go build ./... && go vet ./... && go test ./... && gofmt -l . &&
