...
| `title` | yes (gate) | Single line, ≤120 bytes (the gate measures bytes, so accented or emoji titles hit the limit sooner than the character count suggests). It *is* stored as a separate `title` field, but no query ever targets that field — all retrieval runs against the single corpus (title + description + resource + tags + body) backing both BM25 and the embedding. So title text carries full recall weight through that corpus and gets no boost of its own. Make it the scoped symptom ("KubeContainerOOMKilled for oom-app"), never a vague theme ("OOM issues"). |
| `description` | yes (gate) | One or two sentences carrying the words an alert or a query would contain — prime recall signal. |
| `resource` | Incident: yes (gate) | `namespace/name` of the affected workload, no whitespace. Drives recall's structural workload filter. The gate checks only that it is present (Incident) and whitespace-free — the `namespace/name` shape is what recall *matching* needs, not what the gate enforces. Think hard before omitting it: a resource-less ("scopeless") entry can only be recalled by an incident that itself carries no workload at all, so any namespaced Kubernetes alert will never match it. It stays findable via `kb_search` inside an investigation. |
...
| `status` | no | `active` (or absent) / `retired` / `draft`. `retired` and `draft` cannot fire recall — excluded from both instant recall and the near-miss lead. They stay *searchable* though: an in-investigation `kb_search` still returns them and does **not** print the status, so a retired entry reads there exactly like an active one. (Only the MCP `kb_search`/`kb_get` tools expose `status`.) That is the real reason to fix a wrong entry rather than leave it retired-and-lying. Anything other than those two counts as active. Retire — don't delete — entries that no longer apply. |
