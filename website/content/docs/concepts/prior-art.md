...
|---|---|---|---|
| [**HolmesGPT**](https://github.com/HolmesGPT/holmesgpt) (CNCF Sandbox, Robusta) | Strongest OSS ReAct agent: [56 built-in toolsets](https://holmesgpt.dev/latest/data-sources/builtin-toolsets/) spanning every major observability vendor (Datadog, New Relic, Splunk, Sentry, ES/OS, VictoriaMetrics…), runbooks, MCP client — and since v0.34 an **approval-gated remediation** path (signed approval tickets). | **No** — stateless, human-authored runbooks; every incident starts from zero. | The one to beat on investigation breadth — we can't and shouldn't out-toolset it. What it lacks is ours: **no learning** (its commercial parent sells the intelligence layer), an ArgoCD toolset but **no Flux** (an open PR would add it) and **no revision-exact what-changed diff**. We borrow its toolset + runbook discipline. See [RunLore vs HolmesGPT]({{< relref "/compare/holmesgpt.md" >}}) for the sourced head-to-head. |
...
| [**Aurora**](https://github.com/Arvo-AI/aurora) (Arvo AI, Apache-2.0) | LangGraph agents running kubectl/aws/az/gcloud in sandboxed pods; **deployment diffs**, and it ships **Terraform/IaC tooling**; RAG knowledge base "that grows over time" (Postgres + Weaviate + Memgraph); "Actions": auto-postmortems, **human-reviewed fix PRs** (the user reviews and creates the PR from the UI), Slack. Aggressive comparison-marketing vs Holmes/k8sgpt. | **Yes** — auto-ingested RAG KB, plus ["Aurora Learn"](https://github.com/Arvo-AI/aurora/blob/main/server/routes/incident_feedback/weaviate_client.py): a "helpful" click stores the RCA. But locked in its databases: no review gate, no git export, and the click is a one-time judgement at generation time — never re-checked against whether the fix actually held. | **The fastest-moving OSS threat.** It has diffs, a KB, and PR machinery — combining them into knowledge-PRs is one feature away. Our structural answer: user-owned markdown, human review, outcome weighting. |
