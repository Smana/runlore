#!/usr/bin/env bash
# End-to-end test of RunLore on a throwaway local cluster (k3d or kind).
#
...
#
# Usage: hack/e2e-local.sh [--keep]   (--keep leaves the cluster up for inspection)
#        E2E_PROVIDER=kind hack/e2e-local.sh
#
# E2E_PROVIDER selects the cluster provider: k3d (the default — zero change for
# hack/e2e-k3d.sh users and CI) or kind. Every provider-specific touchpoint lives
# in the provider_* functions below; the test phases are identical on both.
set -euo pipefail

PROVIDER="${E2E_PROVIDER:-k3d}"
case "$PROVIDER" in
  k3d|kind) ;;
  *) printf 'unsupported E2E_PROVIDER %q (want k3d or kind)\n' "$PROVIDER" >&2; exit 2 ;;
esac
...
# ---------------------------------------------------------------------------
# Provider abstraction — the ONLY k3d/kind-specific code in this script.
# ---------------------------------------------------------------------------
...
wait_for_leader() {
  ...
  case "$PROVIDER" in
    k3d)
      ...
      ;;
    kind)
      # No k3d-flag equivalents needed: kind ships no traefik/LB to disable, the
      # suite maps no host ports (it uses `kubectl port-forward`), and kind's
      # default kubelet config already sets the disk-eviction thresholds to 0%
      # (more tolerant of a near-full dev host than the k3s override above).
...
provider_delete_hint() {
  case "$PROVIDER" in
    k3d)  echo "k3d cluster delete $CLUSTER" ;;
    kind) echo "kind delete cluster --name $CLUSTER" ;;
  esac
}

provider_context() {
  # Both CLIs prefix the kubeconfig context with their own name.
  case "$PROVIDER" in
...
# provider_host prints the address at which pods reach the HOST (where the mock
# backends listen). Call it only after the cluster exists.
provider_host() {
  case "$PROVIDER" in
    k3d)
      # k3d injects this alias into CoreDNS, pointing at the host gateway.
      echo "host.k3d.internal"
      ;;
    kind)
      # kind has no such alias on Linux (host.docker.internal is Docker
      # Desktop-only), but pod traffic to the docker "kind" network's gateway
      # lands on the host — so use the gateway IP directly (first IPv4).
...
cleanup() {
  ...
  if [[ "$KEEP" == "0" ]]; then
    ...
    echo "kept cluster '$CLUSTER' and mock (pid $MOCK_PID); delete with: $(provider_delete_hint)"
  fi
}
trap cleanup EXIT
...
provider_delete_cluster
provider_create_cluster
kubectl config use-context "$(provider_context)" >/dev/null
...
HOST="$(provider_host)"   # where in-cluster pods reach the host-side mocks
...
# — harmless here (single-node cluster, no cross-node attach conflict) but unsafe on a
...
# accessModes=[ReadWriteOnce]: both k3d and kind default to a local-path-provisioner
# StorageClass, and local-path doesn't support RWX.
helm upgrade runlore deploy/helm/runlore -n "$NS" --reuse-values \
