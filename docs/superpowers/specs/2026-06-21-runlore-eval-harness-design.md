...
Today none of this is repeatable. **Correction after reading the code (2026-06-21):** a *replay* harness already exists — `internal/eval` (`Case{Tools: map[name]output}` + `staticTool` replay + `Score` keyword-match + `Report`), wired as `lore eval --cases examples/eval`. So the original design's phasing is **inverted in reality**: the Phase-2 replay harness exists; what's missing is the **Phase-1 live-fire layer** and **richer grading**. This spec therefore *extends* `internal/eval` with: live-fire execution against a real cluster, a deterministic **coverage track**, an **LLM-judge** RCA rubric (vs. today's keyword `MustContain`), a **recorder**, and a **report**. Crucially, the recorder emits the **existing `eval.Case` YAML format**, so every live run feeds the replay corpus for free — the two layers compose rather than duplicate.
