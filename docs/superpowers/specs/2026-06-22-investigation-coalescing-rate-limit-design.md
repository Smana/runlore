...
| D6 | **`critical`-severity fast-path in v1** — cooldown is checked *before* the critical case, so the *first* critical for a key flushes immediately while later same-key alerts (critical included) are suppressed | A debounce must never delay the first look at a critical page; but a storm of criticals must still collapse to one investigation, not one per alert (the common `match: [critical]` trigger policy makes this the load-bearing path). |
...
1. `k := key(inc)`.
2. **Cooldown / in-flight** (checked first) — if `recent[k]` is within `cfg.Cooldown`: increment a per-key suppressed counter, emit `alerts_suppressed`, return. No enqueue. Evaluated *before* the critical case so a storm of critical alerts collapses to one investigation (the first) plus suppressions — not one per alert.
3. **Critical fast-path** — else if `inc.Severity == "critical"`: flush immediately (this incident plus any already-pending siblings for `k`), set `recent[k]=now`. The first critical for a key bypasses the debounce; later same-key alerts (critical included) hit the cooldown case above.
