...
func (f *fakeGP) WatchFailures(ctx context.Context) (<-chan providers.FailureEvent, error) {
	return f.ch, nil
}
func (f *fakeGP) Changes(ctx context.Context, w providers.TimeWindow, sel providers.Selector) ([]providers.Change, error) {
	panic("not used")
}
func (f *fakeGP) Diff(ctx context.Context, c providers.Change) (providers.Diff, error) {
	panic("not used")
}
