...
func (li *LoopInvestigator) Investigate(ctx context.Context, req Request) error {
	...
	// originating workload, augmented by what_changed and the gitops inspector tools.
	// An executable action may only target one of these (see downgradeUnobservedTargets).
...
func (li *LoopInvestigator) reviewActions(ctx context.Context, proposed []providers.Action) []providers.Action {
	...
	// F2: a target the model named but no read tool observed server-side is treated as
	// injection — downgrade it to a non-executable suggestion before the policy review.
	proposed = downgradeUnobservedTargets(ctx, proposed, li.Log)
