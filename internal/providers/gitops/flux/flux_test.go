...
// buildMultiCommitRepo creates a temp git repo with two commits touching
// apps/harbor at t=1000 and t=2000, returning the repo dir and the HEAD (v2) SHA.
// The dir doubles as a clonable local RepoURL for the Differ.
...
func TestProviderChangesTimeWindow(t *testing.T) {
	...
	r := fakeReader{
		ks: []kustomization{{
			...
			TargetNamespace: "harbor",
			SourceKind:      "GitRepository", SourceName: "flux-system", SourceNamespace: "flux-system",
			Revision: head, ReadyStatus: "True", ReadyTime: time.Unix(9999, 0),
		}},
		grs: map[string]gitRepository{"flux-system/flux-system": {URL: dir}},
	}
	p := New(r, &whatchanged.Differ{})
	...
	if changes[0].ToRev != head {
		t.Fatalf("newest change ToRev = %q, want HEAD %q", changes[0].ToRev, head)
	}
	...
	if len(changes) != 1 || changes[0].ToRev != head {
